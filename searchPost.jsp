...
<%
	...
	Board[] boardList = bd.searchRecord(key);
...
<html>
	<head>
	<meta charset="UTF-8">
	<title><%=session.getAttribute("ID")%>님의 게시물</title>
...
<!-- 게시글 검색 영역 -->
		<div class="card menuBar search">
			...
			<form class="searchForm"action="searchPost.jsp">
				<input type="text" class="form-control" placeholder="검색할 단어를 입력하세요" name="key" value="<%=key %>">
...
<!-- 게시판 영역 -->
			<div class="board card">
				<div class="card-header">
<%
	if(key.equals(session.getAttribute("ID")+""))
	{
%>
					<h5 class="card-title"><%=session.getAttribute("ID")%>님의 게시물</h5>
<%
	}
	else
	{
